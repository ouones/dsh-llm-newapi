...
function resolveModelReasoning(provider, entry, discovered) {
	...
	if (efforts === void 0) return { reasoning: discovered?.reasoning ?? false };
...
function assertTextOnly(blocks) {
	if (contentHasImage(blocks)) throw new LlmError("The New API adapter does not support image content.", "UNSUPPORTED_CONTENT");
}
/**
...
*/
function serializeMessages(messages, anthropic) {
	const wire = [];
...
*/
function serializeChatRequest(options, model, reasoningEffort) {
	const messages = [];
	if (options.system !== void 0) messages.push({
		role: model.compat.supportsDeveloperRole === true ? "developer" : "system",
		content: options.system
	});
	messages.push(...serializeMessages(options.messages, false));
...
*/
function serializeAnthropicRequest(options, model, _reasoningEffort) {
	const messages = serializeMessages(options.messages, true);
	const tools = options.tools?.map((tool) => ({
		name: tool.name,
		description: tool.description,
...
async function* translateAnthropic(payloads) {
	...
	for await (const payload of payloads) {
		...
		switch (event.type) {
			...
			case "ping": break;
			case "error": throw new LlmError(event.error?.message ?? "Anthropic messages stream error", event.error?.type?.toUpperCase() ?? "API_ERROR");
		}
	}
...
function describableReasoningLevel(model, effort) {
	...
	return offered.length > 0 ? offered[0] : void 0;
}
...
function resolveReasoningEffort(model, profile, requested) {
	...
	if (wire === null || wire === void 0) {
		...
		throw new LlmError(`New API provider "${model.provider}" model "${model.id}" does not support reasoning effort "${level}"`, "UNSUPPORTED_REASONING_EFFORT");
	}
	return {
		wire,
		offered: true
	...
	async *stream(options) {
		...
		try {
			...
			try {
				...
				if (containsImage) throw new LlmError("newapi wire routes are text-only", "UNSUPPORTED_CONTENT");
				...
				const body = model.api === "anthropic-messages" ? serializeAnthropicRequest(options, wireModelOf(model), reasoning.wire) : serializeChatRequest(options, wireModelOf(model), reasoning.wire);
				let response;
				try {
				...
				if (!response.ok) {
					...
					throw new LlmError(text.length > 0 ? `${url} answered ${response.status}: ${text.slice(0, 200)}` : `${url} answered ${response.status}`, response.status === 401 || response.status === 403 ? "AUTH" : response.status === 429 ? "RATE_LIMIT" : response.status >= 500 ? "SERVER" : "INVALID_REQUEST", { status: response.status });
				}
